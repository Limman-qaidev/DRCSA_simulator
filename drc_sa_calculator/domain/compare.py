...
def compare_scenarios(baseline: ScenarioResult, scenario: ScenarioResult) -> ScenarioComparison:
    """Create a comparison data structure between baseline and another scenario."""
    ...
    baseline_map = {exp.trade_id: exp.capital_charge for exp in baseline.exposures}
    for exposure in scenario.exposures:
        base_charge = baseline_map.get(exposure.trade_id, 0.0)
        exposure_deltas[exposure.trade_id] = exposure.capital_charge - base_charge
