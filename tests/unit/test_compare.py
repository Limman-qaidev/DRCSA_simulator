...
def test_compare_scenarios_reports_deltas(
    ...
    stress_charge = next(
        exp.capital_charge
        for exp in result.scenarios[0].exposures
        if exp.trade_id == "T2"
    )
    baseline_charge = next(
        exp.capital_charge
        for exp in result.baseline.exposures
        if exp.trade_id == "T2"
    )
    assert comparison.exposure_deltas["T2"] == pytest.approx(
        stress_charge - baseline_charge
    )
