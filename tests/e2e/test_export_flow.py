    ...
    while time.time() < deadline:
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
            try:
                ...
                time.sleep(0.05)
    raise TimeoutError(
        f"Server on port {port} did not start within {timeout} seconds"
    )
...
def api_server() -> str:
    ...
    thread = threading.Thread(
        target=server.run, name="uvicorn-test-server", daemon=True
    )
    thread.start()
    _wait_for_port(port)
    try:
...
def test_export_flow_via_rest(
    ...
    with _playwright.sync_playwright() as pw:
        request_context: APIRequestContext = pw.request.new_context(
            base_url=api_server
        )
        try:
            for scenario in (baseline_scenario, stress_scenario):
                ...
                response = request_context.put(
                    f"/scenarios/{scenario.name}", json=payload
                )
            ...
            stress_payload = _ensure_ok(
                request_context.get(f"/scenarios/{stress_scenario.name}")
            )
            ...
            baseline_total = compute_response["result"]["baseline"][
                "total_capital_charge"
            ]
            stress_total = compute_response["result"]["scenarios"][0][
                "total_capital_charge"
            ]
